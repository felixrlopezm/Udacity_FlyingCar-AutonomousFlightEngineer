...
import msgpack
import numpy as np
import planning_utils_graph as pu
...
class MotionPlanning(Drone):
    ...
    def __init__(self, connection, data, grid, edges, north_offset, east_offset):
    ...
    def plan_path(self):
        ...
        # Create a graph
        graph = pu.create_graph(edges, start, goal)

        # Plan the path from graph with A* algoritum
        path, _ = pu.a_star_graph(graph, pu.Euclidean_d, start, goal)
        ...
        self.waypoints = [[int(p[0]) + north_offset, int(p[1]) + east_offset, FLIGHT_ALTITUDE, 0] for p in pruned_path]
...
if __name__ == "__main__":
    ...
    # Create a grid at Flight Altitude with defined safety margin around obstacles
    grid, edges, north_offset, east_offset = pu.create_grid_and_edges(data, args.flight_altitude, args.safety_distance)
    
    # Drone Connection
    print('Starting connection to Drone')
    conn = MavlinkConnection('tcp:{0}:{1}'.format(args.host, args.port), timeout=60)
    drone = MotionPlanning(conn, data, grid, edges, north_offset, east_offset)
    time.sleep(1)

...
{
  // This function implements a complementary filter-type attitude filter
  // 
  // The original code, which is commented, implements a small-angle approximation integration method.
  ...
  // 
  // A better integration method requires the use of a Nonlinear integration method that uses the current attitude estimate
  // (rollEst, pitchEst and ekfState(6)) to integrate the body rates into new Euler angles.
  //
  // REMARKS:
  //  - there are several ways to proceed, including:
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // LINEAR COMPLEMENTRY FILTER --> for small angle gyro integration
  // --------------------------
  //
  ...
  // ------------------------------
  // Implementation using quaternion approach

  // Quaternion of the Euler angles taken from state = previous estimate (in world frame)
  Quaternion<float> qt;
  qt = qt.FromEuler123_RPY(rollEst, pitchEst, ekfState(6));

  // Quaternion of the angle change = INTEGRATION of the angular rates taken from IMU in dtIMU (in body frame)
  Quaternion<float> dq;
  dq = dq.FromEuler123_RPY(gyro.x * dtIMU, gyro.y * dtIMU, gyro.z * dtIMU);
  //dq = dq.IntegrateBodyRate(gyro, dtIMU);                   // Alternative expression
    
  // Estimated quaternion as eq. 43 in "Estimation for Quadrotors" paper (q in world frame)
  // Remember that a quaternion multiplication is equivalent to combine two rotations.
  // And since qt represents the attitude estimate of the drone in world frame and dq is the change of that
  // attitude in body frame, therefore, qt_bar is the new attitude in world frame
  Quaternion<float> qt_bar;
  qt_bar = dq * qt;
    
  // Extraction of estimated roll, pitch and yaw angles from the estimated quaternion (in world frame)
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  // REMARKS:
  // - dt is the time duration for which we predict. It will be very short (on the order of 1ms)
  //   so simplistic integration methods are fine here
  // - The Attitude Quaternion from the current state is provided.
  // - Function attitude.Rotate_BtoI(<V3F>) is used
  //   to rotate a vector from body frame to inertial frame
  // - The yaw integration was already done in the UpdateFromIMU function. Therefore, it is not integrated  again here.

  Quaternion<float> attitude = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, curState(6));

  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////

  // Predicted state from current state + velocity integration in dt
  ...
  predictedState(5) = curState(5) + (attitude.Rotate_BtoI(accel).z - 9.81) * dt;
    
  predictedState(6) = curState(6);        // Yaw integration already done in the UpdateFromIMU function
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  // as eq. 52 in "Estimation for Quadrotors" paper
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // - find the current estimated attitude in variables rollEst, pitchEst, state(6).
  // - use the class MatrixXf for matrices. To create a 3x5 matrix A, use MatrixXf class.
  // - the transition model covariance, Q, is loaded up from a parameter file in member variable Q
  // - for transposing a matrix in-place, use A.transposeInPlace(), not A = A.transpose()
  // 
  ...
  // Filling gPrime positions different from Identity matrix
  gPrime(0,3) = dt;
  gPrime(1,4) = dt;
  gPrime(2,5) = dt;

  // Note that we command x-y-z accellerations = u)
  gPrime(3, 6) = (RbgPrime(0) * accel).sum() * dt;
  gPrime(4, 6) = (RbgPrime(1) * accel).sum() * dt;
  gPrime(5, 6) = (RbgPrime(2) * accel).sum() * dt;
    
  // As line 4 in EKF algorithm pseudocode in "Estimation for Quadrotors" paper
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // As eq. 53 from "Estimation for Quadrotors" paper
  zFromX(0) = ekfState(0);
  zFromX(1) = ekfState(1);
  zFromX(2) = ekfState(2);
  zFromX(3) = ekfState(3);
  ...
  // As eq. 55 "Estimation for Quadrotors" paper
  hPrime(0, 0) = 1;
  hPrime(1, 1) = 1;
  hPrime(2, 2) = 1;
  hPrime(3, 3) = 1;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // MAGNETOMETER UPDATE
  // REMARKS:
  //  - current estimated yaw can be found in the state vector: ekfState(6)
  //  - normalize the difference between your measured and estimated yaw
  //  - the magnetomer measurement covariance is available in member variable R_Mag
    
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////

  // As eq. 56 from "Estimation for Quadrotors" paper
  zFromX(0) = ekfState(6);
  
  // normalize distance between estimated and measured yaw to to -pi .. pi
  if (z(0) - zFromX(0) < -F_PI) z(0) += 2.f*F_PI;
  if (z(0) - zFromX(0) > F_PI) z(0) -= 2.f*F_PI;
    
  // As eq. 58 from "Estimation for Quadrotors" paper
  hPrime(0,6) = 1;
